...
export default function WeddingWebsite() {
  ...
  const WEDDING_SIDE = 'Bride';

  // ===========================================================================
  // 📸 CLOUDINARY PHOTO UPLOAD — Pre-filled with your account
  // ===========================================================================
  const CLOUDINARY_CLOUD_NAME = 'dojezyuyf';
  ...
  const [rsvp, setRsvp] = useState({
    name: '', attending: '', guests: 1, meal: 'veg', accommodation: 'no',
    events: [], travel: '', phone: '', message: '',
  });
  const [rsvpSubmitted, setRsvpSubmitted] = useState(false);
  const [rsvpSubmitting, setRsvpSubmitting] = useState(false);
  const [rsvpError, setRsvpError] = useState(null);
  const [uploadStatus, setUploadStatus] = useState({ uploading: false, success: 0 });
  const [liveUpdates, setLiveUpdates] = useState([
    { id: 1, time: 'Posted just now', title: 'Welcome! / स्वागत है', body: 'The countdown has begun. We can\'t wait to celebrate with you. Bookmark this page for live updates during the wedding week. / शुभ घड़ी की प्रतीक्षा शुरू हो गई है।', type: 'info' },
  ]);
  ...
  const openCloudinaryWidget = () => {
    if (typeof window.cloudinary === 'undefined') {
      alert('Upload widget loading... please try again in 2 seconds. / Widget लोड हो रहा है, कृपया 2 सेकंड बाद try करें।');
      return;
    }

    setUploadStatus({ uploading: true, success: 0 });

    const widget = window.cloudinary.createUploadWidget(
      {
        cloudName: CLOUDINARY_CLOUD_NAME,
        uploadPreset: CLOUDINARY_UPLOAD_PRESET,
        folder: 'prakash-priya-wedding',
      ...
      (error, result) => {
        if (error) {
          ...
          setUploadStatus({ uploading: false, success: 0 });
          return;
        }
  ...
  const submitRsvp = async () => {
    ...
    const payload = {
      name: rsvp.name, side: WEDDING_SIDE, attending: rsvp.attending,
      guests: rsvp.guests, meal: rsvp.meal, events: rsvp.events,
      accommodation: rsvp.accommodation, travel: rsvp.travel,
      phone: rsvp.phone || '', message: rsvp.message,
    };
  ...
  return (
    <div className="min-h-screen bg-[#FDF6E9] text-stone-800" style={{ fontFamily: '"Cormorant Garamond", "Tiro Devanagari Hindi", serif' }}>
      <style>{`
        ...
        .mandala-bg { background-image: radial-gradient(circle at 20% 20%, rgba(184, 91, 31, 0.06) 0%, transparent 50%), radial-gradient(circle at 80% 80%, rgba(139, 0, 0, 0.05) 0%, transparent 50%), radial-gradient(circle at 50% 50%, rgba(218, 165, 32, 0.04) 0%, transparent 60%); }
        .paper-texture { background-color: #FDF6E9; background-image: radial-gradient(at 88% 12%, rgba(255,237,213,0.5) 0px, transparent 50%), radial-gradient(at 2% 92%, rgba(254,215,170,0.4) 0px, transparent 50%); }
        .marigold-border { background-image: linear-gradient(90deg, #F59E0B 25%, #DC2626 25%, #DC2626 50%, #F59E0B 50%, #F59E0B 75%, #DC2626 75%); background-size: 16px 100%; }
        ...
        .gold-shimmer { background: linear-gradient(90deg, #B45309 0%, #FBBF24 50%, #B45309 100%); background-size: 200% auto; -webkit-background-clip: text; background-clip: text; color: transparent; animation: shimmer 4s linear infinite; }
        @keyframes fade-up { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
        .fade-up { animation: fade-up 0.8s ease-out both; }
        .ornament::before, .ornament::after { content: "❋"; color: #B45309; margin: 0 1rem; font-size: 0.7em; vertical-align: middle; }
        ...
        .diya-pattern { background-image: repeating-linear-gradient(45deg, transparent, transparent 20px, rgba(180, 83, 9, 0.03) 20px, rgba(180, 83, 9, 0.03) 40px); }
      `}</style>
      ...
      <nav className="fixed top-2 left-0 right-0 z-40 bg-[#FDF6E9]/95 backdrop-blur border-b border-amber-200/50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          <button onClick={() => scrollTo('home')} className="flex items-center gap-2 group">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-amber-600 to-red-700 flex items-center justify-center text-white font-deco text-sm shadow-md group-hover:scale-110 transition">P&P</div>
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navItems.map((item) => (
              <button key={item.id} onClick={() => scrollTo(item.id)}
                className={`px-3 py-2 text-sm font-deco transition-all rounded ${activeSection === item.id ? 'text-red-900 bg-amber-100' : 'text-stone-600 hover:text-red-900 hover:bg-amber-50'}`}>
                {item.label}
              </button>
            ))}
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setMusicOn(!musicOn)} className="p-2 rounded-full hover:bg-amber-100 transition">
        ...
        {navOpen && (
          <div className="lg:hidden border-t border-amber-200 bg-[#FDF6E9]">
            {navItems.map((item) => (
              <button key={item.id} onClick={() => scrollTo(item.id)}
                className={`w-full text-left px-6 py-3 border-b border-amber-100 flex justify-between items-center ${activeSection === item.id ? 'bg-amber-50 text-red-900' : 'text-stone-700'}`}>
      ...
      <section id="home" className="relative pt-24 sm:pt-28 min-h-screen flex items-center mandala-bg paper-texture overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12 text-center relative z-10">
          <div className="fade-up font-hindi text-amber-700 text-sm sm:text-base mb-6 tracking-widest">॥ श्री गणेशाय नमः ॥</div>
          <div className="fade-up" style={{ animationDelay: '0.2s' }}>
            <div className="font-deco text-xs sm:text-sm text-amber-800 tracking-[0.3em] mb-2">TOGETHER WITH THEIR FAMILIES</div>
            <div className="font-hindi text-xs sm:text-sm text-amber-800 mb-4">अपने परिवारों के साथ</div>
            <div className="space-y-2 sm:space-y-4">
              <h1 className="font-script text-5xl sm:text-7xl md:text-8xl gold-shimmer leading-none">{WEDDING.bride.nameEn}</h1>
              <div className="flex items-center justify-center gap-4 my-3">
                <div className="h-px w-12 sm:w-24 bg-amber-600/40" />
                <div className="font-deco text-2xl sm:text-3xl text-red-900">&amp;</div>
                <div className="h-px w-12 sm:w-24 bg-amber-600/40" />
              </div>
              <h1 className="font-script text-5xl sm:text-7xl md:text-8xl gold-shimmer leading-none">{WEDDING.groom.nameEn}</h1>
            </div>
            <div className="font-hindi text-base sm:text-xl text-stone-700 mt-6">{WEDDING.bride.name} &nbsp;◆&nbsp; {WEDDING.groom.name}</div>
          </div>
          <div className="fade-up mt-10 inline-block" style={{ animationDelay: '0.4s' }}>
            <div className="flex items-center gap-4 sm:gap-6 justify-center">
              ...
              <div className="font-serif-display text-6xl sm:text-8xl text-red-900 leading-none px-4 border-x-2 border-amber-600/30">19</div>
          ...
          <div className="fade-up mt-12" style={{ animationDelay: '0.6s' }}>
            <div className="text-xs font-deco text-amber-800 tracking-widest mb-1 ornament"><span>THE COUNTDOWN BEGINS</span></div>
            ...
            <div className="grid grid-cols-4 gap-2 sm:gap-4 max-w-md mx-auto">
              {[
                ...
                <div key={i} className="bg-white/60 backdrop-blur border border-amber-200 rounded-lg p-2 sm:p-4 shadow-sm">
                  <div className="font-serif-display text-2xl sm:text-4xl text-red-900 countdown-num">{String(b.val).padStart(2, '0')}</div>
          ...
          <div className="fade-up mt-10 flex flex-wrap items-center justify-center gap-3" style={{ animationDelay: '0.8s' }}>
            <button onClick={() => scrollTo('rsvp')} className="group inline-flex items-center gap-2 bg-red-800 hover:bg-red-900 text-white px-6 py-3 rounded-full font-deco text-sm tracking-widest shadow-lg hover:shadow-xl transition">
              <Heart size={16} className="group-hover:scale-110 transition" />
              SAVE THE DATE / तिथि सुरक्षित करें
            </button>
            <button onClick={() => scrollTo('events')} className="inline-flex items-center gap-2 bg-white border-2 border-amber-600 text-amber-900 hover:bg-amber-50 px-6 py-3 rounded-full font-deco text-sm tracking-widest transition">
              <Calendar size={16} />
              VIEW EVENTS / कार्यक्रम देखें
            </button>
            <button onClick={shareOnWhatsApp} className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-full font-deco text-sm tracking-widest transition">
              SHARE / साझा करें
            </button>
          </div>
          <div className="fade-up mt-10 font-deco text-sm text-amber-800 tracking-widest" style={{ animationDelay: '1s' }}>{WEDDING.hashtag}</div>
        </div>
      </section>
      ...
      <section id="story" className="py-20 sm:py-28 px-4 sm:px-6 bg-gradient-to-b from-[#FDF6E9] to-amber-50/40">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="font-hindi text-base sm:text-lg text-stone-700 italic mb-4">"{WEDDING.quote}"</p>
          <p className="font-serif-display text-lg sm:text-xl text-stone-700 italic mb-8">"{WEDDING.quoteEn}"</p>
          <p className="font-hindi text-base sm:text-lg text-stone-700 leading-relaxed max-w-3xl mx-auto mb-6">{WEDDING.storyHi}</p>
          <p className="font-serif-display text-lg sm:text-xl text-stone-700 leading-relaxed max-w-3xl mx-auto">{WEDDING.storyEn}</p>
          <div className="mt-16 grid sm:grid-cols-3 gap-6 text-left">
            {[
              { icon: '✨', titleEn: 'How we met', titleHi: 'पहली मुलाक़ात', bodyEn: 'Two families came together through shared connections. The first meeting felt like home.', bodyHi: 'दो परिवार साझे रिश्तों के माध्यम से एक हुए। पहली मुलाक़ात ऐसी लगी जैसे अपना घर हो।' },
              { icon: '👰🤵', titleEn: 'A New Beginning', titleHi: 'नई शुरुआत', bodyEn: 'With the blessings of their families, Priya and Prakash are ready to begin this beautiful journey together.', bodyHi: 'परिवारों के आशीर्वाद के साथ, प्रिया और प्रकाश अब अपनी नई जिंदगी की शुरुआत करने जा रहे हैं।' },
              { icon: '🎉', titleEn: 'Lifetime Commitment', titleHi: 'जीवनभर का साथ', bodyEn: 'Now we invite you to witness our forever begin — with sindoor, vows, and a lot of joy.', bodyHi: 'अब हम आपको आमंत्रित करते हैं कि हमारे सदा-सर्वदा की शुरुआत के साक्षी बनें।' },
            ].map((s, i) => (
      ...
      <section id="events" className="py-20 sm:py-28 px-4 sm:px-6 diya-pattern">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="space-y-5">
            {EVENTS.map((ev) => (
              <div key={ev.key} className={`relative rounded-2xl border-2 p-5 sm:p-7 shadow-sm hover:shadow-lg transition-all ${colorClasses[ev.color]} ${ev.highlight ? 'ring-2 ring-offset-2 ring-red-300' : ''}`}>
                <div className="grid sm:grid-cols-[auto_1fr_auto] gap-4 sm:gap-6 items-start">
                  ...
                  <div className="space-y-2">
                    ...
                    {ev.tithi && <div className="font-hindi text-xs italic opacity-70">({ev.tithi})</div>}
                    <div className="flex items-start gap-2 text-sm opacity-80"><Calendar size={14} className="mt-1 flex-shrink-0" /><span>{ev.time}</span></div>
                    <div className="flex items-start gap-2 text-sm opacity-80">
                      <MapPin size={14} className="mt-1 flex-shrink-0" />
                      <div><div className="font-medium">{ev.venue}</div><div className="text-xs">{ev.venueLine2}</div></div>
                    </div>
                    <div className="flex items-start gap-2 text-sm opacity-80"><Sparkles size={14} className="mt-1 flex-shrink-0" /><span><span className="font-deco text-xs tracking-widest">DRESS / पोशाक:</span> {ev.dress}</span></div>
                    <div className="flex items-start gap-2 text-sm opacity-80"><Phone size={14} className="mt-1 flex-shrink-0" /><span>{ev.contact}</span></div>
                  </div>
                  <div className="flex sm:flex-col gap-2 mt-2 sm:mt-0">
                    <button onClick={() => addToCalendar(ev)} className="text-xs font-deco tracking-widest px-3 py-2 rounded bg-white/70 hover:bg-white border border-current/30 transition">+ CALENDAR / कैलेंडर</button>
                    <a href={ev.mapUrl} target="_blank" rel="noopener noreferrer" className="text-xs font-deco tracking-widest px-3 py-2 rounded bg-white/70 hover:bg-white border border-current/30 transition text-center">MAP / नक्शा →</a>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section id="rsvp" className="py-20 sm:py-28 px-4 sm:px-6 bg-gradient-to-b from-amber-50/40 to-[#FDF6E9]">
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-10">
            ...
            <p className="font-serif-display text-stone-600 mt-1">Kindly respond by <span className="font-semibold">31 May 2026</span></p>
          </div>
          {!rsvpSubmitted ? (
            <div className="bg-white border-2 border-amber-200 rounded-2xl p-6 sm:p-8 shadow-md space-y-5">
              <div>
                ...
                <input type="text" value={rsvp.name} onChange={(e) => setRsvp({ ...rsvp, name: e.target.value })} placeholder="Full name / पूरा नाम"
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[{ val: 'yes', label: 'Joyfully Accept', sub: 'सहर्ष स्वीकार' }, { val: 'no', label: 'Regretfully Decline', sub: 'क्षमा करें' }].map((opt) => (
                    <button key={opt.val} onClick={() => setRsvp({ ...rsvp, attending: opt.val })}
                      className={`p-4 rounded-lg border-2 transition text-left ${rsvp.attending === opt.val ? 'border-red-700 bg-red-50' : 'border-amber-200 hover:border-amber-400'}`}>
                ...
                <>
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      <button onClick={() => setRsvp({ ...rsvp, guests: Math.max(1, rsvp.guests - 1) })} className="w-10 h-10 rounded-full border-2 border-amber-400 hover:bg-amber-100 font-bold">−</button>
                      <div className="font-serif-display text-3xl text-red-900 w-12 text-center">{rsvp.guests}</div>
                      <button onClick={() => setRsvp({ ...rsvp, guests: Math.min(20, rsvp.guests + 1) })} className="w-10 h-10 rounded-full border-2 border-amber-400 hover:bg-amber-100 font-bold">+</button>
                  ...
                  <div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-2">
                      {EVENTS.map((ev) => (
                        <label key={ev.key} className={`flex items-center gap-2 p-3 rounded-lg border cursor-pointer transition ${rsvp.events.includes(ev.key) ? 'border-red-700 bg-red-50' : 'border-amber-200 hover:border-amber-400'}`}>
                          <input type="checkbox" checked={rsvp.events.includes(ev.key)}
                            onChange={(e) => {
                              const events = e.target.checked ? [...rsvp.events, ev.key] : rsvp.events.filter((k) => k !== ev.key);
                              setRsvp({ ...rsvp, events });
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[{ val: 'veg', label: '🌱 Veg' }, { val: 'jain', label: '🙏 Jain' }].map((m) => (
                        <button key={m.val} onClick={() => setRsvp({ ...rsvp, meal: m.val })} className={`p-3 rounded-lg border-2 transition ${rsvp.meal === m.val ? 'border-red-700 bg-red-50' : 'border-amber-200'}`}>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[{ val: 'yes', label: 'Yes, please / हाँ' }, { val: 'no', label: 'No, thanks / नहीं' }].map((a) => (
                        <button key={a.val} onClick={() => setRsvp({ ...rsvp, accommodation: a.val })} className={`p-3 rounded-lg border-2 transition ${rsvp.accommodation === a.val ? 'border-red-700 bg-red-50' : 'border-amber-200'}`}>
                  ...
                  <div>
                    ...
                    <input type="text" value={rsvp.travel} onChange={(e) => setRsvp({ ...rsvp, travel: e.target.value })} placeholder="e.g. Arriving 18 Jun by train"
                  ...
                  <div>
                    ...
                    <input type="tel" value={rsvp.phone} onChange={(e) => setRsvp({ ...rsvp, phone: e.target.value })} placeholder="+91 ____ ___ ___"
                      className="w-full px-4 py-3 border border-amber-300 rounded-lg bg-amber-50/30 focus:outline-none focus:ring-2 focus:ring-amber-600" />
                  </div>
                </>
              )}
              <div>
                ...
                <textarea value={rsvp.message} onChange={(e) => setRsvp({ ...rsvp, message: e.target.value })} rows={3} placeholder="Send your blessings 💌 / आशीर्वाद एवं शुभकामनाएँ"
              ...
              {rsvpError && <div className="bg-red-50 border border-red-300 text-red-800 px-4 py-3 rounded-lg text-sm">⚠️ {rsvpError}</div>}
              <button onClick={submitRsvp} disabled={!rsvp.name || !rsvp.attending || rsvpSubmitting}
            ...
            <div className="bg-white border-2 border-emerald-300 rounded-2xl p-8 text-center shadow-md">
              <div className="w-16 h-16 mx-auto rounded-full bg-emerald-100 flex items-center justify-center mb-4"><Check size={32} className="text-emerald-700" /></div>
              ...
              <p className="font-serif-display text-stone-700 leading-relaxed">
                {rsvp.attending === 'yes' ? `We can't wait to celebrate with you, ${rsvp.name}!` : `Thank you for letting us know, ${rsvp.name}. 💛`}
              </p>
              <p className="font-hindi text-stone-600 leading-relaxed mt-3">
                {rsvp.attending === 'yes' ? `${rsvp.name} जी, आपके साथ इस ख़ुशी के पल मनाने का इंतज़ार है!` : `${rsvp.name} जी, सूचित करने के लिए धन्यवाद। 💛`}
              </p>
              <button onClick={() => { setRsvpSubmitted(false); setRsvpError(null); setRsvp({ name: '', attending: '', guests: 1, meal: 'veg', accommodation: 'no', events: [], travel: '', phone: '', message: '' }); }}
          ...
          <div className="grid md:grid-cols-2 gap-5">
            <div className="bg-white border border-amber-200 rounded-2xl p-6 shadow-sm">
              ...
              <div className="space-y-2 text-sm text-stone-700">
                <div><strong>Patna Airport (PAT)</strong> / पटना हवाई अड्डा — Main hub</div>
            ...
            <div className="bg-white border border-amber-200 rounded-2xl p-6 shadow-sm">
              ...
              <div className="space-y-2 text-sm text-stone-700">
                <div><strong>Patna Jn (PNBE)</strong> / पटना जं — main station</div>
                <div><strong>Rajendra Nagar Terminal (RJPB)</strong> / राजेन्द्र नगर</div>
                <div><strong>Danapur (DNR)</strong> / दानापुर</div>
                <div><strong>Daniyawan</strong> / दनियावां</div>
              </div>
            </div>
            <div className="bg-gradient-to-br from-amber-100 to-orange-50 border border-amber-200 rounded-2xl p-6 shadow-sm">
              ...
              <div className="text-sm text-stone-700 space-y-1">
                ...
                <div className="font-hindi text-xs">तापमान 32–38°C · उमस, हल्की बारिश</div>
            ...
            <div className="bg-gradient-to-br from-rose-50 to-amber-50 border border-amber-200 rounded-2xl p-6 shadow-sm">
              ...
              <div className="text-sm text-stone-700 space-y-1">
                <div><strong>Ola/Uber outstation</strong></div>
                <div><strong>Local taxi, bus</strong></div>
                <div className="text-xs mt-3 italic">Family cars available for pickup.<br /><span className="font-hindi">परिवार की गाड़ियाँ उपलब्ध रहेंगी।</span></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="gallery" className="py-20 sm:py-28 px-4 sm:px-6 bg-gradient-to-b from-[#FDF6E9] to-rose-50/30">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="aspect-square rounded-lg bg-gradient-to-br from-amber-100 via-rose-100 to-orange-100 border border-amber-200 flex items-center justify-center text-4xl hover:scale-105 transition cursor-pointer shadow-sm"
          ...
          <div className="mt-8 text-center space-y-4">
            ...
            {uploadStatus.uploading && (
              <div className="bg-amber-50 border border-amber-300 text-amber-800 px-4 py-3 rounded-lg text-sm max-w-md mx-auto">
                ⏳ Uploading photos... / फ़ोटो अपलोड हो रही हैं...
              </div>
            )}
            {uploadStatus.success > 0 && !uploadStatus.uploading && (
              <div className="bg-emerald-50 border border-emerald-300 text-emerald-800 px-4 py-3 rounded-lg text-sm max-w-md mx-auto">
                ✓ {uploadStatus.success} photo(s) uploaded! Thank you 💛<br />
                <span className="font-hindi">{uploadStatus.success} फ़ोटो अपलोड हुई! धन्यवाद 💛</span>
              </div>
            )}

            <p className="text-xs text-stone-600 font-serif-display italic max-w-md mx-auto">
              Tap the button → choose photos from your phone → they'll upload automatically!<br />
              <span className="font-hindi not-italic">बटन दबाएँ → अपनी फ़ोटो चुनें → स्वतः अपलोड हो जाएँगी!</span>
            </p>
      ...
      <section id="gifts" className="py-20 sm:py-28 px-4 sm:px-6">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="font-serif-display text-stone-600 mt-1">Your presence is the greatest gift.</p>
          </div>
          <div className="max-w-xl mx-auto">
            <div className="bg-gradient-to-br from-rose-50 to-amber-50 border-2 border-rose-200 rounded-2xl p-8 text-center shadow-sm">
              ...
              <p className="text-sm text-stone-700 mb-2">Leave a wish in our RSVP form ❤️</p>
              <p className="font-hindi text-sm text-stone-600 mb-4">हमारे RSVP form में अपना आशीर्वाद छोड़ें ❤️</p>
              <button onClick={() => scrollTo('rsvp')} className="inline-flex items-center gap-2 bg-red-800 hover:bg-red-900 text-white px-6 py-3 rounded-full font-deco text-sm tracking-widest shadow-md transition mt-2">
                LEAVE A BLESSING →
              </button>
            </div>
          </div>
      ...
      <section id="updates" className="py-20 sm:py-28 px-4 sm:px-6 bg-gradient-to-b from-rose-50/30 to-[#FDF6E9]">
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-10">
            ...
            <p className="font-serif-display text-stone-600 mt-1">Real-time announcements during the wedding week</p>
          </div>
          <div className="space-y-3">
            {liveUpdates.map((u) => (
              <div key={u.id} className={`border-l-4 rounded-r-lg p-4 shadow-sm ${u.type === 'rsvp' ? 'border-emerald-500 bg-emerald-50' : u.type === 'alert' ? 'border-red-500 bg-red-50' : 'border-amber-500 bg-amber-50'}`}>
          ...
          <div className="mt-8 text-center text-xs text-stone-600 font-serif-display italic">
            Updates posted by family during the wedding week.<br />
            <span className="font-hindi not-italic">शादी सप्ताह के दौरान परिवार द्वारा अद्यतन।</span>
          </div>
            ...
            <div className="space-y-3">
              {WEDDING.contacts.map((c, i) => (
                <div key={i} className="flex justify-between items-center text-sm border-b border-amber-100 last:border-0 pb-2 last:pb-0">
                  ...
                  <a href={`tel:${c.phone.replace(/\s/g, '')}`} className="font-mono text-sm text-red-800 hover:underline flex items-center gap-1">
      ...
      <footer className="bg-gradient-to-b from-[#FDF6E9] to-red-900 text-amber-100 py-12 px-4 mt-10">
        <div className="max-w-4xl mx-auto text-center">
          <div className="font-script text-4xl gold-shimmer mb-2">{WEDDING.bride.nameEn} &amp; {WEDDING.groom.nameEn}</div>
          <div className="font-deco text-sm tracking-widest text-amber-300 mb-6">19 · 06 · 2026 · PATNA</div>
          <div className="font-hindi text-amber-200 text-sm mb-6">॥ आपकी स्नेहिल उपस्थिति ही हमारे लिए सबसे बड़ा उपहार है ॥</div>
          <div className="flex justify-center gap-4 mb-6"><Heart size={16} className="text-amber-300" fill="currentColor" /></div>
          <p className="text-xs text-amber-300/80 font-serif-display">Made with love by the Singh family · {WEDDING.hashtag}</p>
          <p className="font-hindi text-xs text-amber-300/80 mt-1">सिंह परिवार द्वारा प्रेम से निर्मित</p>
        </div>
